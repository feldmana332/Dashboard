...
> This site lives in the `signups/` subfolder of the repo. The unrelated GRE
> "word of the day" emailer lives at the repo root and keeps working on its own
> GitHub Actions schedule. When deploying to Vercel, point the project's **Root
> Directory** at `signups` (see below) so it only builds this site.
...
3. **Set the Root Directory to `signups`** (Vercel shows this on the import
   screen, or under Project → Settings → Build & Deployment). This is what keeps
   Vercel building only the site and ignoring the Python script at the root.
4. Add the same environment variables from step 2 in the Vercel project settings.
   (`NEXT_PUBLIC_SITE_URL` should be your `https://...vercel.app` URL.)
5. Deploy. Then run the table setup once against your database — either
